  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Morning Briefing</h1>
          <p className="text-slate-400 mt-2">Here are the top opportunities found by the Growth DNA algorithm.</p>
        </div>
        <Badge variant={error ? 'warning' : 'success'} className="px-4 py-2 text-sm">
          {loading ? (
            ...
            `Live Data: Updated at ${formatTime(lastUpdated)}`
          ) : (
            'System Status: Ready'
          )}
        </Badge>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {stocks.map((stock) => (
            <Card key={stock.id} className="p-6 flex flex-col group relative overflow-hidden">
              ...
              >
                Scan DNA Match <ArrowRight className="w-4 h-4" />
              </Link>
            </Card>
          ))}
        </div>
      ...
      <div className="bg-gradient-to-r from-indigo-900/50 to-slate-900/50 border border-indigo-500/20 rounded-xl p-6 flex items-start gap-4">
        ...
        <div>
          <h3 className="text-lg font-semibold text-white mb-1">Market Insight</h3>
          <p className="text-slate-400 text-sm leading-relaxed max-w-3xl">
            Today's scan identified stocks with high volume momentum and growth patterns similar to early-stage tech giants. 
            The AI analyzes price action, volume trends, and fundamental ratios to calculate DNA scores.
          </p>
        </div>
      </div>
    </div>
