...
class WinSourceConnector:
    ...
    def _get_mock_data(self, query: str) -> List[Dict]:
        # Return Deep Link only - no fake stock/price data
        return [{
            "distributor": "Win Source Electronics",
            "mpn": query.upper(),
            "manufacturer": "Various",
            "stock": 0,  # Unknown - check website
            "price": 0,  # Quote required
            "currency": "USD",
            "condition": "Check Website",
            "risk_level": "Medium",
            "source_type": "Deep Link",
            "description": "Click to check availability on Win Source",
            "delivery": "Check Website",
            "date_code": "N/A",
            "datasheet": f"https://www.win-source.net/search/{query}"
        }]
