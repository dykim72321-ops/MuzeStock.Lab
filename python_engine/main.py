...
def analyze_stock(request: AnalyzeRequest):
    ...
    try:
        ...
        macd_signal = macd_indicator.macd_signal().iloc[-1] if len(close) >= 26 else None
        ...
        if sma_20 is not None and sma_50 is not None:
            if sma_20 > sma_50:
                ...
                reasoning.append("20일 이동평균선이 50일선 아래에 있어 하락 추세입니다.")
        
        if macd is not None and macd_signal is not None:
            if macd > macd_signal:
                reasoning.append("MACD가 시그널선 위로 골든크로스 발생 가능성이 있습니다.")
            else:
